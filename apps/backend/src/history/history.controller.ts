...
export class HistoryController {
  ...
  @ApiOperation({
    summary: '月次記録日一覧',
    description: '指定月に何らかの記録（食事 / 体調 / 筋トレ）が存在する日付の配列を返す。',
  })
  ...
  @ApiOperation({
    summary: '日次記録取得',
    description: '指定日の食事・体調・筋トレを一括取得する。存在しないリソースは null。',
  })
  @ApiQuery({ name: 'date', example: '2026-07-04', description: '取得したい日 (YYYY-MM-DD)' })
  ...
  @ApiOperation({
    summary: '日次記録削除',
    description: '指定日の食事・体調・筋トレをトランザクションで一括削除する。存在しない場合も 204 を返す。',
  })
  @ApiQuery({ name: 'date', example: '2026-07-04', description: '削除したい日 (YYYY-MM-DD)' })
  async deleteDaily(
    @CurrentUserId() userId: number,
    @Query('date') date: string,
  ): Promise<void> {
