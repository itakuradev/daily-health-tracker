...
<#
.SYNOPSIS
    ローカル開発専用 Cognito User Pool（shared 層）に開発用ユーザーを作成／更新する。

.DESCRIPTION
    infra/terraform/shared で管理するローカル開発専用 Cognito にのみ開発ユーザーを作成する。
    environments/dev の Cognito は対象にしない（dev 用は scripts/seed-cognito-dev.ps1）。

    - 対象は shared の Terraform output（cognito_local_user_pool_id）が指す User Pool だけ。
      一度の実行で local と dev の両方へ作成することはない。
    - 作成するのは Cognito 側のユーザーのみ。アプリDBの User は作成しない
      （認証後の初回 API アクセス時にバックエンドが自動作成する。認証・認可設計書 10.3 / 18.2）。
    - User Pool ID は shared の Terraform output から取得し、Region は User Pool ID から導出する。
...
.EXAMPLE
    # リポジトリルートで実行する
    .\scripts\seed-cognito-local.ps1 -Email "developer@example.com" -Name "開発ユーザー"
...
# 対象は shared（ローカル開発専用 Cognito）。dev は対象にしない。
$repoRoot = Split-Path -Parent $PSScriptRoot
$tfDir = Join-Path $repoRoot 'infra/terraform/shared'
...
# --- 3. shared の Terraform output から User Pool ID を取得 ----------------
# ローカル開発専用 User Pool（cognito_local_user_pool_id）だけを対象にする。
Write-Section 'Terraform output からローカル用 User Pool ID を取得します'

$poolResult = Invoke-Cli -Exe 'terraform' -CliArgs @("-chdir=$tfDir", 'output', '-raw', 'cognito_local_user_pool_id')
if ($poolResult.ExitCode -ne 0 -or [string]::IsNullOrWhiteSpace($poolResult.StdOut)) {
    Fail "Terraform output から cognito_local_user_pool_id を取得できませんでした。`n$($poolResult.StdErr)" 'shared で terraform apply 済みか、正しいディレクトリかを確認してください。'
}
...
if ([string]::IsNullOrWhiteSpace($region) -or $region -eq $userPoolId) {
    Fail "User Pool ID から Region を導出できませんでした: $userPoolId" 'cognito_local_user_pool_id の値が想定形式か確認してください。'
}
Write-Host "User Pool ID (local): $userPoolId"
...
Write-Host @"
1. frontend / backend のローカル .env に shared の Terraform output の値を設定する
   （output と .env 変数の対応は docs/11-aws-architecture.md「ローカル開発専用 Cognito」を参照）
   例: terraform -chdir=infra/terraform/shared output
...
作成した Email: $Email
（このユーザーはローカル開発専用 Cognito に作成されました。dev 環境には作成していません。）
