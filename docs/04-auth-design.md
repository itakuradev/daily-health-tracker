...
* 認証実装の範囲を最小化し、アプリ本体の設計・実装に注力できる
...
日次記録画面では、食事・体調・筋トレなど複数の認証必須APIが同時に呼ばれる。同じAccess Tokenを付与した複数リクエストが同時に401を返すと、各リクエストが個別にToken更新やログアウトを実行し、`/oauth2/token`への不要な多重アクセスやログアウトの多重実行が起こりうる。

そのため、次を保証する。

* `forceRefresh`は、同じ古いTokenに由来する一連の401に対して**原則1回だけ**実行する
* 更新中は更新Promiseを複数リクエストで共有する
* 待機していたリクエストは、更新後の同じAccess Tokenでそれぞれ**1回だけ**再送する
* 各リクエストの再送は最大1回とし、無限再試行しない
* 更新後の再送でもなお401なら、ログアウト処理は**一度だけ**開始する

**遅延401の考慮**：更新が完了しPromiseが解放された後に、別リクエストの401が遅れて返ることがある。単純な「実行中Promiseの共有」だけではこの遅延401で二重更新が起こりうる。そこで、リクエストがTokenを取得した時点の世代を記録し、そのリクエストの送信後にすでにTokenが更新済みであれば、再度`forceRefresh`せず更新後のTokenで再送する。

### Token更新失敗時の分類

`fetchAuthSession({ forceRefresh: true })`の失敗は、すべてを認証切れとして扱わない（UserInfo・JWT検証と同じ考え方。「17.4」「17.6」を参照）。

Amplify自身が失敗原因を分類しており（`TokenOrchestrator`）、次の公式契約に従う。

| Amplifyの挙動                              | 意味                     | フロントの扱い |
| -------------------------------------- | ---------------------- | ------- |
| Token不在のセッションで解決（Amplifyがtokenをクリア）    | Refresh Token失効等、再ログインが必要 | ログアウトを一度だけ開始し、一貫した認証エラー（401相当）を返す |
| 例外を送出（通信・サービス障害、または予期しないエラー）           | 一時障害／分類不能             | ログアウトしない。一時障害エラー（503相当）へ変換する |

方針：

* 再ログインが必要と確実に判断できる場合のみログアウトする
* ネットワーク障害・Cognitoの一時障害・分類不能な例外では、ログアウトも認証状態の破棄もしない
* エラーメッセージの文字列判定に依存せず、「token不在で解決＝要再ログイン」「throw＝一時障害/分類不能」という公式契約で分類する
* いずれの場合も、Access Tokenや内部エラーの詳細を画面へ出さない

なお、Hubの`tokenRefresh_failure`イベントは再ログイン要否にかかわらず発火するため、これを直接ログアウトの契機にはしない。

### ログアウト処理の集約

`onUnauthorized`やログアウトが複数リクエストから同時に呼ばれても、実際の`signOut()`は一度だけ開始する。ログアウト完了後・再サインイン完了後には集約用の制御状態をリセットし、以降のログアウトや更新が永久に抑止されないようにする。
...
## 17.4 認証情報の無効と外部サービス障害の区別

バックエンドがCognitoのUserInfoエンドポイントを呼び出す際、失敗を**すべて401にしない**。

401は「認証情報が無効」を意味し、フロントエンドはこれを受けてtoken更新と再試行を行い、なお401であればログアウトする（「17.1」を参照）。

そのため、Cognitoの一時的な障害を401として返すと、**正当なユーザーが記録の入力途中で強制ログアウトされる**。

したがって、原因に応じて次のように分類する。

| 原因                              | 分類          | ステータス |
| ------------------------------- | ----------- | ----- |
| Access Tokenが不正・期限切れ            | 認証情報が無効     | 401   |
| UserInfoが401 / 403を返した（tokenを拒否） | 認証情報が無効     | 401   |
| UserInfoが5xxを返した                | 外部サービスの一時障害 | 503   |
| UserInfoが429など上記以外のエラーを返した      | 外部サービスの一時障害 | 503   |
| UserInfoへ接続できない・タイムアウト          | 外部サービスの一時障害 | 503   |
| UserInfoが200系だがJSONとして解釈できない    | 外部サービスの不正応答 | 502   |
| UserInfoのレスポンス形式が期待と異なる         | 外部サービスの不正応答 | 502   |
| DB接続エラー・想定外の内部エラー               | 内部エラー       | 500   |

方針：

* 401は「tokenが無効」と判断できる場合に限定する
* 外部サービス起因の失敗は5xxとし、フロントエンドが認証切れと誤認しないようにする
* DBエラーや想定外の例外は認証エラーへ変換せず、そのまま上位へ送出する
* いずれの場合も、レスポンスに内部エラー・Access Token・UserInfoの個人情報を含めない
* 失敗の理由はサーバー側のログにのみ記録する

```mermaid
flowchart TD
  Call["UserInfo呼び出し"] --> Conn{"接続できた？"}
  Conn -->|No| Unavailable["503 一時障害"]
  Conn -->|Yes| Status{"ステータスは？"}
  Status -->|"401 / 403"| Unauth["401 認証情報が無効"]
  Status -->|"その他のエラー"| Unavailable
  Status -->|"200系"| Parse{"JSON・形式は妥当？"}
  Parse -->|No| BadGateway["502 不正応答"]
  Parse -->|Yes| Ok["User作成へ進む"]
```

## 17.5 UserInfoレスポンスの検証

UserInfoのレスポンスは外部から受け取る値であるため、型を仮定せず実行時に検証してからUser作成に用いる。

検証項目：

```text
1. nullや配列ではないobjectであること
2. subが空白でない文字列であること
3. subが検証済みAccess Tokenのsubと一致すること
4. emailが空白でない文字列であること
5. nameが文字列または未設定であること
```

いずれかを満たさない場合はUserを作成せず、外部サービスの不正応答として扱う（502）。

`sub`の一致確認を行う理由：

Access Tokenから取得した`sub`と、UserInfoが返した`sub`が異なる場合、別のユーザーの属性を取り込んでUserを作成してしまう恐れがある。
`User.cognitoSub`はAccess Token側の`sub`を用いるため、不一致のまま作成すると、`cognitoSub`と`email`が別人の組み合わせになる。

`email`が取得できない場合は、架空の値を保存せずUserを作成しない（「10.3」を参照）。

## 17.6 JWT検証時の無効Tokenと外部障害の区別

バックエンドがAccess TokenのJWT検証を行う際、失敗を**すべて401にしない**。

JWKS取得などの外部障害を401にすると、フロントエンドが認証切れと誤認し、正当なユーザーをログアウトさせてしまう（「17.1」を参照）。そのため、UserInfoの場合（「17.4」）と同じ考え方でJWT検証の失敗も分類する。

JWT検証には `aws-jwt-verify` を用いる。同ライブラリは失敗を型付きの例外として投げるため、**例外クラスへの `instanceof` 判定**で分類する（メッセージ文字列での判定は行わない）。

| 原因                                    | 分類          | ステータス |
| ------------------------------------- | ----------- | ----- |
| JWT形式が不正・署名が不正・署名アルゴリズムが想定外          | Tokenが無効     | 401   |
| 有効期限切れ・token_use不一致・client_id不一致などclaim検証エラー | Tokenが無効     | 401   |
| issuer不一致                             | Tokenが無効     | 401   |
| 有効なkidがない・最新JWKSにも存在しないkid            | Tokenが無効     | 401   |
| JWKSエンドポイントへの接続失敗・タイムアウト             | 外部サービスの一時障害 | 503   |
| JWKS取得のバックオフ中・キャッシュ未取得               | 外部サービスの一時障害 | 503   |
| 取得したJWKS/JWKの内容が不正                    | 外部サービスの一時障害 | 503   |
| 上記いずれにも安全に分類できない例外                    | 内部エラー       | 500   |

方針：

* Tokenが無効と断定できる例外のみ401とし、`verify()` が `null` を返して `AuthGuard` が401にする
* JWKS取得・外部通信の障害は503として送出し、`AuthGuard` は401へ変換しない
* どちらにも安全に分類できない例外は、認証失敗として握りつぶさず元の例外を再送出する（500）
* 401・503・500のいずれもレスポンスに内部情報・Token・claimを含めない
* 失敗の理由は、Token全文やclaimを含めず、例外クラス名のみをサーバー側ログに記録する
* 無効Tokenと外部障害は、運用上区別できるようログレベルを分ける（無効Token: warn / 外部障害: error）

補足（issuer不一致の扱い）：

`aws-jwt-verify` のCognito向け検証では、issuer不一致は専用のissuer例外ではなく、引数・設定系の例外（"issuer not configured"）として投げられる。

一方、`userPoolId` / `clientId` などの構築時バリデーションは検証器の生成時（起動時）に発生し、Token検証時の失敗経路には現れない。そのため、Token検証時に到達するこの例外はTokenのissuerに起因するものに限られ、Token無効（401）として扱う。

```mermaid
flowchart TD
  Verify["JWT検証（aws-jwt-verify）"] --> Result{"例外の型は？"}
  Result -->|"Token無効系"| Null["verify() が null → 401"]
  Result -->|"JWKS取得/外部通信系"| Unavailable["503 一時障害（AuthGuardは401化しない）"]
  Result -->|"分類不能"| Internal["元の例外を再送出 → 500"]
```
...
ただし、dev側でCognitoの設定変更を繰り返し検証する間はdev側に置く利点もあるため、導入時期は別途判断する。
