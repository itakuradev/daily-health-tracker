...
> **v2に関する注記**
> 画面レイアウト・UIは v2 で刷新している（共通ヘッダー、日付ポップオーバー、履歴画面の週間グラフ、記録画面の3カラムカード、体調スコアUIの変更など）。本書の画面仕様のうちレイアウト・UI表現に関する記述は、最新の内容を「UI設計書 v2（14）」で確認すること（→14参照）。データ項目・保存条件・削除方針などの業務仕様は本書のまま有効。
